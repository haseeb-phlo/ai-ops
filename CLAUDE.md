...
**New Superset worktrees bootstrap themselves** via `.superset/config.json` (committed). Its `setup` array runs in a terminal in the new worktree on creation, `&&`-joined so a failure short-circuits: it copies `.env.local` from `$SUPERSET_ROOT_PATH` (the main checkout at `~/.superset/projects/ai-ops`, where the canonical gitignored env file lives) and then runs `npm ci`. That install step checks its own work: an `npm ci` that exits fine but leaves `node_modules/.bin` empty is reinstalled from scratch once, and a second empty `.bin` fails setup loudly. A half-populated `node_modules` is otherwise invisible until some later `npm run` dies with `sh: next: command not found`. `run` is `npm run dev`, launched by the Run button in its own pane. Superset reads this config from the **main checkout only** - a copy edited inside a worktree has no effect until it's merged to `main`; to test a change first, use the per-machine override at `~/.superset/projects/<projectId>/config.json`. Without this, a fresh worktree has no env file and every page 500s with "Your project's URL and Key are required to create a Supabase client".
