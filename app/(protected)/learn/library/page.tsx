...
import { canManageLibrary } from "@/lib/programme/learn-access";
import {
  LEARN_SUBTOPICS,
...
/**
 * The raw video library: every Learn video, grouped by topic, with the tools
 * to add, edit, attach to and reorder them.
 *
 * ADMIN ONLY, and that is the whole reason this route exists. This used to be
 * /learn itself, open to all 138 people - which meant a cohort member on day
 * one could see and tick off day eleven's video, and anybody at all could
 * watch the programme's content without ever doing the check-in that measures
 * whether it worked. /learn is now the gate; the library it used to be lives
 * here, for the people who have to maintain it.
 *
 * Members reach these same videos through /learn/track, in the order and at
 * the pace the programme intends.
 */
export default async function LibraryPage() {
  const user = await getSessionUser();
  if (!canManageLibrary(user.realRole)) redirect("/learn");
