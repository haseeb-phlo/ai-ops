 ...
 * name Cowork at all, so for the task the reference is gone entirely. If the ordering is ever
 * revisited, this is the argument for putting Scheduled Tasks back behind
 * Cowork, and the reason not to is the video binding above.
 *
 * That gap was one day when this swap was made and is four now, because the
 * 8/11 swap below moved Cowork to day 11. Do not read "the day after" into
 * this note: it has already been wrong once.
 *
 ...
 * contradiction to smooth over. Cowork is day 11 since the 8/11 swap, so this
 * pairing now spans a week rather than a day, and the two are further apart
 * than the lesson wants.
 *
