...
type SendInterventionCommentArgs = {
  recipient: Recipient;
...
/**
 * Notifies one participant in an AI-initiative comment thread that a new
 * comment has arrived. Headline differs slightly for the initiative's owner
 * ("on your AI initiative") vs. a prior commenter ("on an AI initiative you
 * commented on") so the recipient instantly knows why they're being emailed.
 *
 * No-ops if Resend isn't configured.
 */
export async function sendInterventionCommentEmail({
  recipient,
  interventionId,
  interventionName,
  commentBody,
  ...
  isOwner,
  appUrl,
}: SendInterventionCommentArgs): Promise<
...
> {
  ...
  const link = `${appUrl.replace(/\/+$/, "")}/interventions/${encodeURIComponent(interventionId)}`;
  const firstName = recipient.displayName.split(" ")[0] || recipient.displayName;
  const intro = isOwner
    ? "left a comment on your AI initiative"
    : "left a comment on an AI initiative you commented on";

  const subject = `${commenterName} commented on "${interventionName}"`;
  const text = [
    `Hi ${firstName},`,
    "",
    `${commenterName} ${intro} "${interventionName}":`,
    "",
    commentBody,
    "",
    `Reply: ${link}`,
        ...
        <tr>
          <td style="padding:20px 28px 4px 28px;">
            <h1 style="margin:0;font-size:20px;font-weight:600;letter-spacing:-0.015em;color:${PHLO_NAVY};line-height:1.3;">
              New comment on &ldquo;${escapeHtml(interventionName)}&rdquo;
            </h1>
          </td>
        </tr>
        <tr>
